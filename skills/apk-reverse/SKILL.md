---
name: apk-reverse
description: Android APK 逆向：解包、jadx 反编译、smali 修改、重打包签名安装、Frida 动态 Hook，以及按需切换到 .so/native 分析。Android APK reversing — unpack, jadx decompile, smali patching, repack/sign/install, Frida hooking, native .so analysis.
license: MIT
---
...
本技能依赖 `jadx`、`apktool`、`frida-ps`、`adb`、`java`。各工具的可用性与实际路径以 `../tool-index.md` 为准；缺失时调用 bootstrap 补齐，不猜路径。
